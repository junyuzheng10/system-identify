#!/usr/bin/env python
"""System identification from logged data (marvinM6 right-arm single-arm variant).

Same pipeline as identify_params.py, adapted for the replay_right_arm_002_log_data
logs: 7 right-arm joints (j0-j6), and the command trajectory may be logged at a
different rate than the sensors (interpolated onto sensor timestamps).
"""
import argparse
import numpy as np
import pandas as pd
...
def main():
    ...
    # ===== Validation on a separate dataset (optional) =====
    if args.val_dir is not None:
        ...
        logger.info("VALIDATION: 3-way RMS error comparison")
        logger.info(f"{'Joint':>6} | {'RMS meas':>10} | {'Nominal err':>12} {'ratio':>7} | {'Ident err':>12} {'ratio':>7}")
        logger.info("-" * 80)
        for j in range(njoints):
        ...
        fig_val.suptitle(f"VALIDATION ({args.val_dir}): Measured vs Nominal (RNEA) vs Identified Torque")
        plt.tight_layout()
        ...
        fig_acc_val.suptitle(f"VALIDATION ({args.val_dir}): Expected Acceleration (blue) vs {acc_label} (red)")
        plt.tight_layout()
